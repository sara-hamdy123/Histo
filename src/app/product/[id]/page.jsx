  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative w-full h-[60vh] text-white flex items-center justify-center">
        ...
        />
        <h1 className="absolute top-[20%] left-[5%] text-[3vw] md:text-[64px] font-bold">
          {product.title}
        </h1>
      </div>
      {/* Product Details */}
      <div className="mx-auto shadow-lg flex flex-col md:flex-row items-center justify-between p-6 md:p-10 bg-gradient-to-r from-[#f0e68c] via-[#f5f5f5] to-[#AA8E5C]">
        <h2 className="text-3xl md:text-4xl font-bold w-full md:w-[360px] text-center md:text-left mb-6 md:mb-0">
          Discover the {product.title}
        </h2>
        <p className="text-xl w-full md:w-[870px] text-center md:text-left">{product.description}</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row container mx-auto mt-32 justify-between">
        <div className="flex flex-col w-full lg:w-[550px]">
          <h2 className="font-bold text-3xl">Exploring the Lifestyle of the {product.title}</h2>
          <p className="py-5 leading-6">{product.description}</p>
        </div>
        <ImgRelated />
      </div>
