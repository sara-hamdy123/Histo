...
export default function FixedInfo() {
  ...
  useEffect(() => {
    async function fetchProduct() {
      ...
      try {
        ...
        setVisibleImages(productImages.slice(0, 3)); // عرض أول 3 صور بشكل افتراضي
      } catch (error) {
  ...
  const handleRadioChange = (index) => {
    ...
    setMainImage(images[index]); // تغيير الصورة الرئيسية عند الضغط على الراديو
  };
    ...
    <div className="min-h-screen flex flex-col mx-auto container p-4 md:p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 my-6">
        {visibleImages.map((src, index) => (
          <div key={index} className="relative w-full h-32 sm:h-40 md:h-48 bg-black overflow-hidden rounded-xl cursor-pointer"
            onClick={() => openSlider(index)}>
            <Image src={src} alt={`Thumbnail ${index + 1}`} fill className="object-fill opacity-90" />
          </div>
        ))}

        {/* صورة تحتوي على +25 تفتح السلايدر عند الضغط */}
        <div className="relative w-full h-32 sm:h-40 md:h-48 flex items-center justify-center bg-black rounded-xl cursor-pointer"
